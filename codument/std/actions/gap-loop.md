...
如果当前 scope 已由上层 `action-hooks.xml` 或 track.xml 的 `<cdt:AttractorCheck>`/`<cdt:GapLoop>` 拥有校验，**不要**在 gap-loop 子代理内启动 competing nested check。可以读取已有 hook 结果作为背景，但是否重跑由父层决定。
...
- `cdt:TrackLink state="bound"` 能解析到真实 track：`codument/tracks/active/<id>/track.xml` 或 `codument/tracks/archived/**/<timestamp>-<id>/track.xml`。
...
- **缺省取全局默认**：节点未写 `verify-round` 时，取全局默认。**全局默认为 `false`（关）**——约定如此；如 `config/action-hooks.xml` 的 `<Action name="gap-loop">` 显式声明了全局开关，则以该声明覆盖此约定默认。
- **判定**：
...
- `codument/tracks/active/<track-id>/proposal.md`
- `codument/tracks/active/<track-id>/behavior_deltas/**/*.xml`
- `codument/tracks/active/<track-id>/design.md`（如存在）
- `codument/tracks/active/<track-id>/track.xml`
- `codument/tracks/active/<track-id>/reports/` 下已有的历史报告（如存在）
...
---- /?a5
---- #step ?a6
生成新 gap 报告 → track / phase scope 写 `codument/tracks/active/<id>/reports/track-impl-gap-report-<round>.md`；mission scope 写 `codument/missions/<state>/<id>/reports/mission-gap-loop-report-<round>.md`
---- /?a6
...
<codument-gap-loop-result version="1">
  ...
  <report_path>codument/tracks/active/add-user-auth/reports/track-impl-gap-report-4.md</report_path>
